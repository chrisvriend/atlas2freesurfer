...
denoise_protocol=ICAAROMA8Phys # protocol that was used for denoising
###############################
...
if [ ! -f ${fmriprepdir}/${subj}${sessionpath}func/denoised/${subj}${sessionfile}task-rest_space-${outputspace}_desc-smooth_${denoise_protocol}_bold.nii.gz ]; then
