...
#SBATCH --output=atlas2FS_%A_%a.log
...
Usage() {
    ...
    Usage: ./Atlas2FreeSurfer.sh <freesurferdir> <fmriprepdir> <session> 
    Obligatory:
...
afnitools_container=/scratch/anw/share-np/AFNI23.0.7
export APPTAINER_BINDPATH="/scratch/anw,/data/anw/anw-archive,/data/anw/anw-gold"
...
denoise_protocol=24HMP8PhysSpikeRegGS # protocol that was used for denoising
FWHM=6
###############################
...
denoisedimg=${fmriprepdir}/${subj}${sessionpath}func/${subj}${sessionfile}task-rest_space-${outputspace}_rec-susan${FWHM}mm_desc-${denoise_protocol}_bold.nii.gz


if [ ! -f ${denoisedimg}  ]; then
...
for atlas in BNA 300P7N 400P7N; do
	...
	if [ ! -f ${fmriprepdir}/${subj}${sessionpath}anat/${subj}${sessionfile}space-${outputspace}_atlas-${atlas}_dseg.nii.gz ]; then
		...
		apptainer run ${afnitools_container} 3dresample \
			...
			-master ${boldreffile} \
			-prefix \
			${fmriprepdir}/${subj}${sessionpath}anat/${subj}${sessionfile}space-${outputspace}_atlas-${atlas}_temp.nii.gz
	...
	if [[ ${ID} != *"Schaefer"* ]]; then
		...
		labelconvert ${fmriprepdir}/${subj}${sessionpath}anat/${subj}${sessionfile}space-${outputspace}_atlas-${atlas}_temp.nii.gz \
			${atlasdir}/${atlas}/${ID}_orig.txt \
			${atlasdir}/${atlas}/${ID}_modified.txt \
			${fmriprepdir}/${subj}${sessionpath}anat/${subj}${sessionfile}space-${outputspace}_atlas-${atlas}_dseg.nii.gz -force
	else
		labelconvert ${fmriprepdir}/${subj}${sessionpath}anat/${subj}${sessionfile}space-${outputspace}_atlas-${atlas}_temp.nii.gz \
	...
	rm ${fmriprepdir}/${subj}${sessionpath}anat/${subj}${sessionfile}space-${outputspace}_atlas-${atlas}_temp.nii.gz
done
conda deactivate
...
for atlas in BNA 300P7N 400P7N; do
	...
	fslmeants -i ${denoisedimg} \
		--label=${fmriprepdir}/${subj}${sessionpath}anat/${subj}${sessionfile}space-${outputspace}_atlas-${atlas}_dseg.nii.gz \
	...
	fslstats -K ${fmriprepdir}/${subj}${sessionpath}anat/${subj}${sessionfile}space-${outputspace}_atlas-${atlas}_dseg.nii.gz \
